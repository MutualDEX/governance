...
contract BigFeeTo {
    ...
    function setOwner(address owner_) public {
        require(msg.sender == owner, 'BigFeeTo::setOwner: not allowed');
        owner = owner_;
    }

    function setFeeRecipient(address feeRecipient_) public {
        require(msg.sender == owner, 'BigFeeTo::setFeeRecipient: not allowed');
        feeRecipient = feeRecipient_;
    }

    function updateTokenAllowState(address token, bool allowed) public {
        require(msg.sender == owner, 'BigFeeTo::updateTokenAllowState: not allowed');
    ...
    function renounce(address pair) public returns (uint value) {
        ...
        TokenAllowState storage token0AllowState = tokenAllowStates[IBigswapV2Pair(pair).token0()];
        TokenAllowState storage token1AllowState = tokenAllowStates[IBigswapV2Pair(pair).token1()];
        ...
        if (
            ...
            value = IBigswapV2Pair(pair).balanceOf(address(this));
            if (value > 0) {
                ...
                assert(IBigswapV2Pair(pair).transfer(pair, value));
                IBigswapV2Pair(pair).burn(pair);
            }
    ...
    function claim(address pair) public returns (uint value) {
        ...
        if (
            ...
            if (value > 0) {
                ...
                assert(IBigswapV2Pair(pair).transfer(feeRecipient, value));
            }
        }
    }
}

interface IBigswapV2Pair {
